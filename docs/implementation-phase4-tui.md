...
- Fixtures validate (`pnpm run validate`)
- Build and tests pass (`pnpm run build`, `pnpm test`)
...
```bash
pnpm start -- --questionnaire fixtures/basic/simple-text-survey.json
pnpm start -- -q fixtures/advanced/customer-feedback.json --resume <sessionId>
```
...
- [x] Record any other divergences (none found; `pnpm run validate` reports all fixtures valid)
...
- [x] Add `"start": "pnpm run build && node dist/app.js"` — rebuilds then runs
...
# Resume
pnpm start -- -q fixtures/basic/simple-text-survey.json --resume <sessionId printed on Ctrl+C>
```
...
- [ ] `pnpm start -- --questionnaire fixtures/basic/simple-text-survey.json` completes without error (manual interactive verification pending)
...
- [x] TypeScript build (`pnpm run build`) completes with no errors
