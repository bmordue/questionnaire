...
function parseArgs(): { responseFile: string; questionnaireFile: string; outputFile?: string } {
  ...
  if (args.length < 2) {
    ...
    console.error('');
    console.error('Usage:');
    console.error('  pnpm run markdown-convert -- <response.json> <questionnaire.json> [output.md]');
    ...
    console.error('  pnpm run markdown-convert -- response.json questionnaire.json');
    console.error('  pnpm run markdown-convert -- response.json questionnaire.json output.md');
    process.exit(1);
  }
