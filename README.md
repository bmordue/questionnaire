...
```bash
pnpm run build && pnpm run web &       # start the questionnaire service on port 3000
...
```bash
pnpm start -- --questionnaire fixtures/basic/simple-text-survey.json --resume <sessionId>
```

For iterative runs without rebuilding:

```bash
pnpm run start:dev -- --questionnaire fixtures/basic/simple-text-survey.json
```
...
# Output to file
pnpm run markdown-convert -- examples/sample-response.json fixtures/basic/simple-text-survey.json output.md
```
